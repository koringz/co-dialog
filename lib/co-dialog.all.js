...
(function (foctory) {
    !function (factory) {
        ...
        codialog.prototype.hide = function (options) {
            ...
            if(isStr(options)) {
                if (inArray(options, this.cacheDialogElement)) {
                    this.excuteHideAnimation(options + ' [mask]', this.$(options))
                }
            }
        ...
        codialog.prototype.excuteHideAnimation = function (options, currentDialogNode) {
            var self = this;
            ...
            if(document.querySelector && document.addEventListener) {
                ...
                // animation动画加载
                this.animate(options).delay(100).fadeOut({
                    type: 'end',
                    callback: function () {
                        currentDialogNode.style.display = 'none';
                        self.resetScroll(false);
        ...
        ** **/
        codialog.prototype.show = function (options) {
            ...
            if(isStr(options)) {
                if (inArray(options,this.cacheDialogElement)) {
                    this.excuteShowAnimation(options + ' [dialog]', _currentElements);
                }
            }
            else if (isObj(options)) {
                var _timeout = Number(options.timeout);
                ...
                this.excuteShowAnimation(self.dialogElement + ' [dialog]', _currentElements);
            }

            return this;
        }

        codialog.prototype.excuteShowAnimation = function (options, currentDialogNode) {
            var self = this;
        ...
        codialog.prototype.resetScroll = function (isTruth) {
            ...
            if(isTruth) {
                classList(bodyNode, ' codialog-show', document.body);
                classList(document.documentElement, ' codialog-show', document.documentElement);
