...
codialog.prototype.tipEvent = function (obj) {
    if ('event' in obj) this.foreachTipEvent(obj['event'], obj.follow);
    else return false;
}

codialog.prototype.foreachTipEvent = function (getEvt,follow) {
    var self = this;
...
codialog.prototype.tipFollow = function (fow) {
    ...
    if ('follow' in fow) this.dialogTip(fow['follow']);
    else return false;
}

codialog.prototype.dialogTip = function (self) {
    ...
    box.css({position:'absolute','background-color': 'transparent','border-radius':0,margin:0,left:_left + _width,top:_top - box_height/2 + _height/2});
}
