...
codialog.prototype.use = function (obj, success_config) {
    ...
    if (Object.prototype.toString.call(obj) == '[object Object]') {
        ...
        */
        ...
        // tip跟随元素定位
        if (Object.prototype.toString.call(obj.tip) == '[object Object]') {
            if ('bool' in obj.tip) {
                if(obj.tip.bool) {
                    //  清除mask遮罩层
                    $(this.dialogElement).find('[mask]').css({'position':'static'});
                    // 清除header，body，footer元素内容
                    $(this.dialogElement).find(this.strict.header).html('');
                    $(this.dialogElement).find(this.strict.body).html('');
                    $(this.dialogElement).find(this.strict.footer).html('');

                    // 填充提示信息tip message
                    $(this.dialogElement).find('[container]').append(self.viewTipSemantics(obj.tip.message || '提示信息为空'));
                    // follow表示跟随元素的节点信息
                    this.tipFollow(obj.tip);

                    // 绑定事件到处理节点的tip提示信息
                    this.tipEvent(obj.tip);
                }
                else if(!obj.tip.bool) {
                    return console.log('不能显示tip信息');
                }
                else return console.log('tip不正确');
            }
            else return null;
        }
    }
...
codialog.prototype.tipEvent = function (obj) {
    if ('event' in obj) this.foreachTipEvent(obj['event'], obj.follow, {mouseout:obj.mouseout,mouseover:obj.mouseover});
    else return false;
}

codialog.prototype.foreachTipEvent = function (getEvt,follow, mouse) {
    var self = this;

    if (Array.isArray(getEvt)) true;
    else if (typeof getEvt == 'string') getEvt = [getEvt];
   
    $(follow).attr('bind-tip-el',self.dialogElement);
    $(follow).on(getEvt.shift() || 'mouseover', function (ev) {
        ev.preventDefault();
        // 鼠标划过时操作执行函数方法
        if(typeof mouse.mouseover == 'function') mouse.mouseover($(this));
        $($(this).attr('bind-tip-el')).show();
    }).on('mouseout',function (ev) {
        ev.preventDefault();
        // 鼠标划出时操作执行函数方法
        if(typeof mouse.mouseout == 'function') mouse.mouseout($(this));
        var targetElement = $($(this).attr('bind-tip-el'));

        var is = 0; 
        // 鼠标划过提示框显示提示框 反之同理
        targetElement.on('mouseover',function (evc) {
            evc.preventDefault();
            $(this).show();
            is = 1;
        }).on('mouseout',function (evc) {
            evc.preventDefault();
            $(this).hide();
            is = 0;
        });
        ...
        // 直接划出目标节点 隐藏弹出框
        if(this.mouseoutcount) clearTimeout(this.mouseoutcount);
        this.mouseoutcount = setTimeout(function () {
            if(is) is = 0;
            else targetElement.hide();
        },200);
    });
}

codialog.prototype.tipFollow = function (fow) {
    var getFollowElements = null;

    if ('follow' in fow) this.showTip(fow['follow'], fow.pos||'right');
    else return false;
}

codialog.prototype.showTip = function (self,pos) {
    var _left,_top,_width,_height;
    var rectPosition = self.getBoundingClientRect();
    _left = rectPosition.left;
    _top = rectPosition.top;
    _width = rectPosition.width;
    _height = rectPosition.height;
    var box = $(this.dialogElement).find('[dialog]');
    var box_height = box.height();
    var box_width = box.width();

    var dom = $(document);
    var scroll_top = dom.scrollTop();
    var scroll_left = dom.scrollLeft();
    var director = new Object;
    var horizontal_center = 0;
    pos = pos.toLowerCase();
    if(pos == 'left') {
        director = {
            LEFT: _left - box_width + scroll_left,
            TOP: _top - _height/2 + scroll_top
        };
    }
    else if(pos == 'right') {
        director = {
            LEFT: _left + _width + scroll_left,
            TOP: _top - box_height/2 + _height/2 + scroll_top
        };
    }
    else if(pos == 'top') {
        director = {
            LEFT: _left + scroll_left,
            TOP: _top - box_height + scroll_top
        };
        horizontal_center = (box_width - _width)/2;
    }
    else if(pos == 'bottom') {
        director = {
            LEFT: _left + scroll_left,
            TOP: _top + _height + scroll_top
        };
        horizontal_center = (box_width - _width)/2;
    }
    box.css({position:'absolute','background-color': 'transparent','border-radius':0,margin:0,left: director.LEFT,top: director.TOP, marginLeft: -horizontal_center});
}

codialog.prototype.$tip = function (_dom,opt) {
    if (!(Object.prototype.toString.call(_dom).indexOf('HTML') > -1)) opt = _dom;
    else Object.assign(opt,{follow: _dom});
    return this
        .app(opt.create || 'body')
        .use({
            adaptDialogWidth: opt.adaptDialogWidth || true,
            adaptDialogHeight: opt.adaptDialogHeight || true,
            tip: {
                bool: opt.bool || true,
                pos: opt.pos || 'right',
                event: opt.event || 'mouseover',
                follow: opt.follow || document.querySelector('body'),
                message: opt.message || '',
                mouseout: opt.mouseout || function (){},
                mouseover: opt.mouseover || function (){}
            }
        })
...
codialog.prototype.viewTipSemantics = function (params) {
    var str = '';

    str += '<div tipLeft class="default-tool-tip-img-left"></div>';
    str += '<div tipCenter class="default-tool-tip-content default-tool-tip-img-center">';
    str += '<div tipMessage class="default-tool-tip-content-middle">' + params;
    str += '</div>';
    str += '</div>';
    str += '<div tipRight class="default-tool-tip-img-right"></div>';
