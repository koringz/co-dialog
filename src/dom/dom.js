...
codialog.prototype.resetScroll = function (isTruth) {
    if(isTruth) {
        ...
        if(isUndefined(ignoreZoreClass)) return null;
...
codialog.prototype.removeChild = function (child) {
    if(isUndefined(child)) return null;
...
codialog.prototype.classList = function (nowNodeList, params, newNode) {
    if(isStr(params)) {
        if(nowNodeList.classList) {
            nowNodeList.setAttribute('class',(isStr(newNode) ? '' : newNode.classList) + params);
        }
        else if(nowNodeList.className) {
            nowNodeList.className = (isStr(newNode) ? newNode.className : '') + params;
        }
        return null;
    }
...
codialog.prototype.margin = function (obj, currentNode, custom_margin, margin) {
    ...
    if (!isNaN(obj[custom_margin]) || 
        (!isStr(obj[custom_margin]) ? false : obj[custom_margin].search(/^((([\d]+|[\d]+(\em|\px|\%|rem|\ex))?(\s+))*)?([\d]+|[\d]+(\em|\px|\%|rem|\ex))?(\s*)$/g) >= 0)) {
        parse[_margin] = obj[custom_margin];
    }
    else if(!isUndefined(obj[custom_margin]) && Object.prototype.toString.call(obj[custom_margin]) == '[object Object]') {
        if( !isUndefined(obj[custom_margin].horizontal)
            && !isUndefined(obj[custom_margin].vertical)
